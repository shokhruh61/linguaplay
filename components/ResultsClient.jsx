  ...
  return (
    <div className="space-y-6">
        ...
        {result.wrongAnswers?.length ? (
          <ul className="mt-4 space-y-3">
            {result.wrongAnswers.map((item) => {
              ...
              if (!q) return null;
              return (
                <li key={`${item.questionId}-${item.chosen}`} className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm">
      ...
      <section className="flex flex-wrap gap-3">
        <Link href={`/game?level=${level}`} className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800">
          Replay
        </Link>
        <Link href="/levels" className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
          Darajani tanlash
        </Link>
