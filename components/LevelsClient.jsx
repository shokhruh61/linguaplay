    ...
    <div className="space-y-8">
      ...
      <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {LEVELS.map((level) => {
          ...
          return (
            <article key={level} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              {isUnlocked ? (
                <Link href={`/game?level=${level}`} className="mt-4 inline-block rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800">
                  O‘ynash
                </Link>
              ) : (
                <button type="button" disabled className="mt-4 rounded-lg bg-slate-200 px-4 py-2 text-sm font-semibold text-slate-500">
                  Yopiq
                </button>
              )}
